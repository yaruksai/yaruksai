...
def _extract_issues_from_text(auditor_text: str) -> List[ParsedIssue]:
    ...
    # Second pass: multiline YAML-ish blocks
...
# =========================
# MIZAN DECISION POLICY (MVP FAST MODE)
# =========================
def _decide_review(
    mizan_score: int,
    issues: List[ParsedIssue],
    ...
    """
    MVP Fast Policy:
    - Any HIGH issue => revise_required
    - Otherwise => approve_for_build (even if there are MEDIUM/LOW issues)
      Builder should apply accepted_fixes conservatively.
    - If loop limit reached => human_escalation
    """
    ...
    return {
        ...
        "reason": "No high-severity issues; approved for build in MVP policy",
    }
...
def run_mizan_engine(
    ...
    # =============================================================
    # TEST MODE: Fail-closed davranışını doğrulamak için
    # Terminal:
    #   YARUKSAI_FORCE_REVIEW_REQUIRED=1 python src/flows/orchestrator.py
    # =============================================================
    if os.getenv("YARUKSAI_FORCE_REVIEW_REQUIRED", "").strip() == "1":
        return {
            "version": MIZAN_ENGINE_VERSION,
            "timestamp": _utc_now_iso(),
            "agent_id": "mizan_engine_test_override",
            "review_loop_count": review_loop_count,
            "max_review_loops": MAX_REVIEW_LOOPS,
            "mizan_score": 0,
            "review_decision": "review_required",
            "decision_reason": "Forced non-approval for testing fail-closed behavior",
            "issue_count": 1,
            "issues": [
                {
                    "severity": "high",
                    "category": "governance_test",
                    "problem": "Forced non-approval for testing fail-closed.",
                    "fix": "Unset YARUKSAI_FORCE_REVIEW_REQUIRED and rerun.",
                    "source_line": None,
                }
            ],
            "accepted_fixes": [],
            ...
            "yaruksai_context_packet": {
                ...
                "agent_id": "mizan_engine_test_override",
                "mizan_score": 0,
                "architect_summary": (architect_output_text or "")[:200],
                "audit_summary": "",
                "required_fixes": [],
                "top_constraints": [],
                "policy_flags": {
